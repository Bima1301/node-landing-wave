...
export default function Service() {
    return (
        <motion.section className=' mb-20 relative ' initial={{ opacity: 0, y: 100 }}
            ...
            <div className='flex md:flex-row flex-col justify-around mb-12 gap-8 px-[4rem]'>
                {services.map((item, index) => (
                    <div className='flex flex-col gap-5 service-content w-full' key={index}>
                        ...
                        {item.description && (
                            <div className='lg:text-base md:text-sm text-xs' key={index} dangerouslySetInnerHTML={{ __html: item.description }} />
                        )}
                    </div>
                ))}
            </div>
